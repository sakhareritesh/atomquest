...
export default function AdminSharedGoalsPage() {
  ...
  const fetchData = useCallback(async () => {
    if (!user) return;
    try {
      const [cyclesRes, usersRes, taRes] = await Promise.all([
        fetch("/api/cycles"),
        fetch("/api/users"),
      ...
      setAllUsers((usersData.users || []).filter((u: User) => u.id !== user.id));
  ...
  function toggleAll() {
    if (selectedMembers.length === allUsers.length) {
      setSelectedMembers([]);
    } else {
      setSelectedMembers(allUsers.map((m) => m.id));
    }
  }
  ...
  async function handleCreateKpi(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.count > 0) {
          toast.success(`KPI assigned to ${data.count} employee(s)`);
        }
        if (data.skippedCount > 0) {
          toast.info(`${data.skippedCount} employee(s) already had this goal`);
        }
        if (data.lockedSkippedCount > 0) {
          toast.warning(`${data.lockedSkippedCount} employee(s) skipped (sheet locked/full)`);
        }
        resetForm();
  ...
  return (
    <div>
      <Header title="Shared Goals (Admin)" />
      <div className="p-6 space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <Plus className="h-5 w-5" /> Create Departmental KPI
            </CardTitle>
            <CardDescription>
              Create a KPI and assign it across the organization.
              Recipients can only adjust the weightage; goal title and target are read-only.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleCreateKpi} className="space-y-6">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {allUsers.length > 0 && (
                    <Button type="button" variant="ghost" size="sm" onClick={toggleAll}>
                      {selectedMembers.length === allUsers.length ? "Deselect All" : "Select All"}
                    </Button>
                  )}
                </div>
                {allUsers.length > 0 ? (
              ...
              >
                ...
                {submitting ? "Creating..." : `Create & Assign KPI to ${selectedMembers.length} Employee(s)`}
              </Button>
        ...
        {sharedKpis.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                All KPIs created and assigned across the organization.
              </CardDescription>
            </CardHeader>

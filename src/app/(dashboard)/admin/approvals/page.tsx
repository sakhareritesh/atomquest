...
import { CheckCircle, XCircle, Eye, AlertCircle, Clock, Unlock } from "lucide-react";
...
export default function AdminApprovalsPage() {
  ...
  const filteredSheets = filterStatus === "all"
    ? sheets
    : sheets.filter((s) => s.status === filterStatus);
  ...
  async function handleApprove() {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Goal sheet approved and locked for ${selectedSheet.employee?.name}`);
        setDetailOpen(false);
        await fetchSheets();
      } else {
  ...
  async function handleUnlock(sheetId: string) {
    if (!confirm("Unlock this goal sheet? Goals will return to draft status.")) return;
    try {
      const res = await fetch("/api/goal-sheets", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: sheetId, status: "draft" }),
      });
      if (res.ok) {
            ...
            <DialogHeader>
              <DialogTitle className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span>Review: {selectedSheet?.employee?.name}</span>
                  {selectedSheet?.employee?.department && (
                    <Badge variant="outline">{selectedSheet.employee.department}</Badge>
                  )}
                  <Badge variant="secondary" className="capitalize">
                    {selectedSheet?.employee?.role}
                  </Badge>
                </div>
          ...
          <Card className="border-yellow-200">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2">
                ...
                Pending Approvals
                <Badge className="bg-yellow-100 text-yellow-800">{pendingSheets.length}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
            ...
            <CardTitle className="text-lg flex items-center justify-between">
              ...
              <Select value={filterStatus} onValueChange={(v) => { if (v) setFilterStatus(v); }}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="locked" label="Locked">Locked</SelectItem>
          ...
          <CardContent>
            {filteredSheets.length > 0 ? (
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <p className="text-sm text-muted-foreground">
                          ...
                          {sheet.cycle?.name && ` | ${sheet.cycle.name}`}
                          {sheet.reject_reason && (
              ...
              <p className="text-center py-8 text-muted-foreground">
                No goal sheets found with status &quot;{filterStatus}&quot;
              </p>
            )}
          </CardContent>
        </Card>
